humhub.module('onlinedrives', function (module, require, $) {
    ...
    FolderView.prototype.initEvents = function () {
        var that = this;
        event.on('humhub:file:created.onlinedrives', function (evt, files) {
        ...
        event.on('humhub:file:modified.onlinedrives', function (evt, files) {
    ...
    FolderView.prototype.initContextMenu = function () {
        ...
        $("#bs-table tr").contextMenu({
            getMenuSelector: function ($invokedOn, settings) {
                ...
                switch (fileItem.options['onlinedrivesType']) {
                    case "folder-posted":
                ...
                if(fileItem.options['onlinedrivesEditable']) {
    ...
    FolderView.prototype.showUrl = function (item) {
        ...
        options.url = item.options.onlinedrivesUrlFull;

        options.head = item.options.onlinedrivesType === 'folder' ? options.headFolder : options.headFile;
    ...
    FolderView.prototype.loader = function (show) {
        var $loader = this.$.find('#onlinedrives-crumb');
        if (show === false) {
            loader.reset($loader);
            return;
        }
    ...
    FolderView.prototype.hasItems = function () {
        return this.$fileList.find('[data-onlinedrives-item]').length > 0;
    };
    ...
    FileItem.prototype.init = function () {
        this.id = this.$.data('onlinedrives-item');
        this.type = this.$.data('onlinedrives-type');
        this.url = this.$.data('onlinedrives-url');
        this.wallUrl = this.$.data('onlinedrives-wall-url');
        this.editUrl = this.$.data('onlinedrives-edit-url');
        this.moveUrl = this.$.data('onlinedrives-move-url');
    };
    ...
    FileItem.prototype.move = function () {
        var that = this;
        var fid = $('#onlinedrives-folderView').data('fid') || 0;
        modal.global.post({
            'url': that.moveUrl,
            'data': {
                'selection[]': that.id
    ...
    var _getDirectoryList = function () {
        return Widget.instance('#onlinedrives-directory-list');
    };

    var move = function (evt) {
        client.submit(evt, {dataType: 'html'}).then(function (response) {
            modal.global.setDialog(response.html);
            var fid = $('#onlinedrives-folderView').data('fid');
            _getDirectoryList().select(fid);
            modal.global.show();
        }).catch(function (e) {
            module.log.error(e, true);
    ...
    var unload = function () {
        event.off('humhub:file:created.onlinedrives');
        event.off('humhub:file:modified.onlinedrives');
    };
