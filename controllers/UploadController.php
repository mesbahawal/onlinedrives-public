...
use humhub\modules\onlinedrives\actions\UploadAction;
...
class UploadController extends BrowseController
{
    ...
    public function actionImport($fid)
    {
        ...
        foreach ($guids as $guid) {
            $onlineDrive = ModelFile::findOne(['guid' => $guid]);

            if (!$onlineDrive) {
                $errors[] = Yii::t('Onlinedrives.base', 'Could not import file with guid {guid}. File not found', ['guid' => $guid]);
                Yii::error(Yii::t('Onlinedrives.base', 'Could not import file with guid {guid}. File not found', ['guid' => $guid]));
                continue;
            }

            $onlineDrive->show_in_stream = false;

            $file = new File($this->contentContainer);
            $file->setFileContent($onlineDrive);
        ...
        if ($errors) {
            array_unshift($errors, Yii::t('OnlinedrivesModule.base', 'Some files could not be imported: ') );
        }
    ...
    {
        $errorMsg = Yii::t('OnlinedrivesModule.base', 'Some files could not be imported: ');
