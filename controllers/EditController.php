...
use humhub\modules\onlinedrives\models\forms\SelectionForm;
...
class EditController extends BrowseController
{
    ...
    public function actionFolder($id = null)
    {
        ...
        if ($folder->load(Yii::$app->request->post()) && $folder->save()) {
            $this->view->saved();
            return $this->htmlRedirect($folder->createUrl('/onlinedrives/browse/index'));
        }

        return $this->renderPartial('modal_edit_folder', [
            'folder' => $folder,
            'submitUrl' => $this->getCurrentFolder()->createUrl('/onlinedrives/edit/folder', ['id' => $folder->getItemId()])
        ]);
    }
    ...
    public function actionFile($id, $fromWall = 0)
    {
        ...
        if (empty($file) || !($file instanceof File)) {
            throw new HttpException(404, Yii::t('OnlinedrivesModule.base', 'Cannot edit non existing file.'));
        }
        ...
        if ($file->baseFile->load(Yii::$app->request->post()) && $file->baseFile->validate()) {
            ...
            if ($duplicate && !$duplicate->is($file)) {
                $file->baseFile->addErrors(['file_name' => Yii::t('OnlinedrivesModule.base', 'A file with that name already exists in this folder.')]);
            } elseif ($file->load(Yii::$app->request->post()) && $file->save()) {
                if ($fromWall) {
                    return $this->asJson(['success' => true]);
                } else {
                    $this->view->saved();
                    return $this->htmlRedirect($this->contentContainer->createUrl('/onlinedrives/browse/index', ['fid' => $file->parent_folder_id]));
                }
            }
        }
        ...
        return $this->renderPartial('modal_edit_file', [
            'file' => $file,
            'submitUrl' =>  $this->contentContainer->createUrl('/onlinedrives/edit/file', ['fid' => $file->parent_folder_id, 'id' => $file->getItemId(), 'fromWall' => $fromWall]),
        ]);
    }
