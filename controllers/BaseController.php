...
abstract class BaseController extends ContentContainerController
{
            ...
            switch ($folderId) {
                ...
                default:
                    $this->_currentFolder = Folder::find()->contentContainer($this->contentContainer)
                        ->readable()
                        ->where(['onlinedrives_folder.id' => $folderId])
                        ->one();
                    if ($this->_currentFolder === null) {
                        throw new HttpException(500, Yii::t('OnlinedrivesModule.base', 'Could not find folder with id: %id%', ['%id%' => $folderId]));
                    }
            }
        }
    ...
    protected function getFolderList($parentId = self::ROOT_ID, $orderBy = NULL)
    {
        ...
        $foldersQuery->andWhere(['onlinedrives_folder.parent_folder_id' => $parentId]);
    ...
    protected function getItemsList($filesOrder = NULL, $foldersOrder = NULL)
    {
        ...
        $filesQuery->andWhere([
            'onlinedrives_file.parent_folder_id' => $this->getCurrentFolder()->id
        ]);

        // user maintained folders
        $foldersQuery->andWhere([
            'onlinedrives_folder.parent_folder_id' => $this->getCurrentFolder()->id
        ]);
        ...
        $specialFoldersQuery->andWhere(['onlinedrives_folder.parent_folder_id' => $this->getCurrentFolder()->id]);
