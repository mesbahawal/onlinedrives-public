...
use humhub\modules\onlinedrives\models\File as OnlineDrive;
...
class ZIPCreator extends ZipUtil
{
    ...
    public function __construct()
    {
        ...
        if ($code !== true) {
            throw new Exception(Yii::t('OnlinedrivesModule.base', 'Opening archive failed with error code %code%.', ['%code%' => $code]));
        }
    }
    ...
    {
        Yii::info('Written ZIP: ' . $this->tempFile . '(Number of files: ' . $this->archive->numFiles . ' - Status: ' . $this->archive->status . ')', 'onlinedrives');
        $this->archive->close();
    }
    ...
    {
        if ($item instanceof OnlineDrives) {
    ...
    {
        if ($file instanceof OnlineDrive) {
            $file = $file->baseFile;
        }
    ...
    public function addFolder(Folder $folder, $path = '')
    {
        ...
        $subFiles = OnlineDrive::find()->where(['parent_folder_id' => $folder->id])->all();
