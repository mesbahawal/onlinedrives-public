...
class ZipExtractor extends ZipUtil
{
    ...
    public function extract(Folder $targetFolder, UploadedFile $file)
    {
        ...
        if (strtolower($file->extension) === 'zip') {
            ...
            if ($file->saveAs($sourcePath, false)) {
                ...
                $targetFolder->addError('upload', Yii::t('OnlinedrivesModule.base', 'Archive %filename% could not be extracted.', ['%filename%' => $file->name]));
            }
        } else {
            $targetFolder->addError('upload', Yii::t('OnlinedrivesModule.base', '%filename% has invalid extension and was skipped.', ['%filename%' => $file->name]));
        }
    ...
    /**
     * Generate the cfolder and onlinedrive structure in the database of a given folder and all its subfolders recursively.
     *
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($filePath)) {
                ...
                if(!$folder) {
                    ...
                    if(!$folder->save()) {
                        $root->addError('upload',  Yii::t('OnlinedrivesModule.base', 'An error occurred while creating folder {folder}.', ['folder' => $file]));
                        continue;
                    }
                }
                ...
                if($result->hasErrors()) {
                    $root->addError('upload',  Yii::t('OnlinedrivesModule.base', 'An error occurred while unpacking {filename}.', ['filename' => $file]));
                }
            }
        }
    }
    ...
    /**
     * Create a onlinedrive model and create and connect it with its basefile File model from a given data file, connect it with its parent folder.
