...
/**
 * This is the model class for table "onlinedrives_folder".
 *
 * @property integer $id
...
class Folder extends FileSystemItem
{
    ...
    {
        return Yii::t('OnlinedrivesModule.base', "Folder");
    }
    ...
    public function uniqueTitle($attribute, $params, $validator)
    {
        ...
        if ($this->parentFolder->folderExists($this->title)) {
            $this->addError('title', \Yii::t('OnlinedrivesModule.base', 'A folder with this name already exists.'));
        }
    }
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            'id' => 'ID',
            'parent_folder_id' => Yii::t('OnlinedrivesModule.models_Folder', 'Parent Folder ID'),
            'title' => Yii::t('OnlinedrivesModule.models_Folder', 'Title'),
            'description' => Yii::t('OnlinedrivesModule.models_Folder', 'Description')
        ]);
    }
    ...
    public function getVisibilityTitle()
    {
        if (Yii::$app->getModule('friendship')->getIsEnabled() && $this->content->container instanceof User) {
            ...
            return $this->content->isPublic() ? Yii::t('OnlinedrivesModule.base', 'This folder is public.') : $privateText;
        }

        return $this->content->isPublic() ? Yii::t('OnlinedrivesModule.base', 'This folder is public.') : Yii::t('OnlinedrivesModule.base', 'This folder is private.');
    }
    ...
    public function migrateFromOldStructure()
    {
        ...
        $filesQuery = File::find()->joinWith('baseFile')->contentContainer($this->content->container)
            ->andWhere(['OR', ['IS', 'onlinedrives_file.parent_folder_id', new \yii\db\Expression('NULL')], ['onlinedrives_file.parent_folder_id' => 0]]);

        $foldersQuery = Folder::find()->contentContainer($this->content->container)
            ->andWhere(['OR', ['IS', 'onlinedrives_folder.parent_folder_id', new \yii\db\Expression('NULL')], ['onlinedrives_folder.parent_folder_id' => 0]])
            ->andWhere(['IS', 'onlinedrives_folder.type', new \yii\db\Expression('NULL')]);
    ...
    public static function getSubFoldersByParent($parent, $orderBy = ['title' => SORT_ASC])
    {
        ...
        $query->andWhere(['onlinedrives_folder.parent_folder_id' => $parent->id]);

        // do not return any subfolders here that are root or allpostedfiles
        $query->andWhere([
            'or',
            ['onlinedrives_folder.type' => null],
            ['and',
                ['<>', 'onlinedrives_folder.type', Folder::TYPE_FOLDER_POSTED],
                ['<>', 'onlinedrives_folder.type', Folder::TYPE_FOLDER_ROOT]
            ]
        ]);
    ...
    {
        return $this->content->container->createUrl('/onlinedrives/edit/folder', ['id' => $this->getItemId()]);
    }

    public function noSpaces($attribute, $params)
    {
        if (trim($this->$attribute) !== $this->$attribute) {
            $this->addError($attribute, Yii::t('OnlinedrivesModule.base', 'Folder should not start or end with blank space.'));
        }
    }
     ...
     *
     * Returns the newly created onlinedrives file.
